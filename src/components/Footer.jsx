import { TrendingUp, Mail, MapPin, Phone, ChevronRight } from "lucide-react";
import { useApp } from "../context/AppContext";

const footerLinks = {
  Company: ["About Us", "Our Team", "Careers", "Press Room", "Advertise"],
  Categories: [
    "World News",
    "Politics",
    "Technology",
...
export default function Footer() {
  ...
  const handleCategoryClick = (cat) => {
    ...
    const found = validCats.find((c) => cat.includes(c));
    if (found) {
  ...
  return (
    <footer className="bg-slate-900 dark:bg-slate-950 text-gray-400 dark:text-slate-500">
      {/* Top Footer */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-16 h-10 sm:w-20 sm:h-12 md:w-24 md:h-14 rounded-lg overflow-hidden shadow-md group-hover:scale-105 transition-transform">
                <img
            ...
            <p className="text-sm leading-relaxed mb-5 text-gray-500">
              Delivering trusted, accurate, and timely news to millions of
              readers worldwide since 2010.
            </p>
            ...
            <div className="flex gap-2 flex-wrap">
              {socialLinks.map((s) => (
                <a
                  ...
                  href={s.href}
                  title={s.name}
                  className={`w-9 h-9 bg-slate-800 dark:bg-slate-700 ${s.color} text-gray-400 hover:text-white rounded-lg flex items-center justify-center transition-all duration-200 hover:scale-110`}
                >
                  {s.icon}
                </a>
              ))}
                ...
                {links.map((link) => (
                  <li key={link}>
                    <button
                      ...
                      className="text-sm text-gray-500 dark:text-slate-500 hover:text-blue-400 transition-colors flex items-center gap-1 group"
                    >
                      <ChevronRight
                        size={12}
                        className="opacity-0 group-hover:opacity-100 transition-opacity -ml-2 group-hover:ml-0 text-blue-400"
                      />
                      {link}
                    </button>
                  </li>
